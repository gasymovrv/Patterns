...
+ ArrayList
    + лучшее O(1) - чтения из любого места (прямой доступ к памяти через массив) и вставка в начало;
...
+ HashMap
    ...
    + худшее O(n) - чтение, если у всех элементов одинаковый hashCode
    + худшее O(log(n)) - чтение, если у всех элементов одинаковый hashCode - начиная с Java 8 после достижения определенного порога размера корзины вместо связанных списков используются сбалансированные деревья
  	
+ TreeMap
    + гарантировано O(log(n)) - все операции, т.к. красно-черное дерево обеспечивает такую сложность
