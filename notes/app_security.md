...
+ **ЗАЩИТА**:
    + Фильтрация параметров, работающих с базой данных
        ...
        + В Java при передаче параметров в SQL-запрос использовать PreparedStatement - 
        он автоматически экранирует ковычки (Hibernate тоже испольует его)
...
+ **ЗАЩИТА(от кражи сессий)**:
    ...
    2. Защита сессии IDs:
        ...
        + ID сессии должны защищаться флагами ```Secure``` и ```HTTPonly```. 
