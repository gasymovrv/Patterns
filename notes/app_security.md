...
+ **ПРИМЕР**:
    + Общими словами: разместив комментарии со скриптом отправляющим куки или ID сессии пользователя на сторонний сервер злоумышленника.
    + Технически: Внедрение в БД приложения скрипта который при появлении на UI будет совершать вредоносные действия 
